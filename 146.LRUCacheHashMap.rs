...
mod tests {
    ...
    fn test_3() {
        ...
        tests::test_series_operations(operations, operation_nums);
    }
}
    ...
    fn get(&mut self, key: i32) -> i32 {
        if let Some(value) = self.map.clone().get(&key) {
            match (value.prev_key, value.next_key) {
                ...
                (None, Some(next_key)) => {
                    if let Some(next_value) = self.map.clone().get(&next_key) {
                        self.map.insert(next_key, LRUValue { val: next_value.val, prev_key: None, next_key: next_value.next_key });
                        self.head_key = Some(next_key);
                    }
                }
                _ => {}
            }
            if let Some(head_key) = self.head_key {
                if head_key != key {
                    if let Some(head_value) = self.map.clone().get(&head_key) {
                        self.map.insert(head_key, LRUValue { val: head_value.val, prev_key: Some(key), next_key: head_value.next_key });
                        self.map.insert(key, LRUValue { val: value.val, prev_key: None, next_key: Some(head_key) });
                    } else {
                        self.map.insert(key, LRUValue { val: value.val, prev_key: None, next_key: None });
                    }
    ...
    fn put(&mut self, key: i32, value: i32) {
        if self.map.is_empty() {
            ...
            if self.map.len() == self.max_size {
                if let Some(tail_value) = self.map.clone().get(&tail_key) {
                    if let Some(prev_key) = tail_value.prev_key {
                        if let Some(prev_value) = self.map.clone().get(&prev_key) {
                            self.map.insert(prev_key, LRUValue { val: prev_value.val, prev_key: prev_value.prev_key, next_key: None });
                            self.tail_key = tail_value.prev_key;
                        }
                    }

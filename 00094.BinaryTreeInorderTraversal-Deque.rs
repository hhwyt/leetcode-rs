...
impl Solution {
    pub fn inorder_traversal(mut some_root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        ...
        while some_root.is_some() || !queue.is_empty() {
            while let Some(rc_root) = some_root {
                queue.push_back(rc_root.clone());
                some_root = rc_root.borrow().left.clone();
            }

            let rc_root = queue.pop_back().unwrap();
            res.push(rc_root.borrow().val);
            some_root = rc_root.borrow().right.clone();
        }
        res

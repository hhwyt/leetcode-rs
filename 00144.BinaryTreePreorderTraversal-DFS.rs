...
impl Solution {
    fn preorder_traversal_recursive(root: &Option<Rc<RefCell<TreeNode>>>, v: &mut Vec<i32>) {
        match root {
            Some(root) => {
                let root = RefCell::borrow(root);
                v.push(root.val);

                Self::preorder_traversal_recursive(&root.left, v);
                Self::preorder_traversal_recursive(&root.right, v);
            }
            None => {}
        }
    }
    ...
    pub fn preorder_traversal(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        let mut v = Vec::new();
        Self::preorder_traversal_recursive(&root, &mut v);
        v
    }
}
...
mod tests {
    ...
    fn test_normal() {
        ...
        assert_eq!(Solution::preorder_traversal(root), expected);
    }

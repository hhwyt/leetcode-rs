...
impl Solution {
    fn word_break_recursive(s: &str, start: i32, end: i32, word_dict: &Vec<String>) -> bool {
        lazy_static! {
            static ref MEMO: CHashMap<(i32, i32), bool> = Default::default();
        }
        ...
        if let Some(ret) = MEMO.get(&(start, end)) {
            return *ret;
        }

        for word in word_dict {
            if let Some(pos) = s[start as usize..end as usize].find(word) {
                if Self::word_break_recursive(&s, start, start + pos as i32, word_dict)
                    && Self::word_break_recursive(&s, start + (pos + word.len()) as i32, end, word_dict) {
                    MEMO.insert((start, end), true);
                    return true;
                }
            }
        }
    ...
    pub fn word_break(s: String, word_dict: Vec<String>) -> bool {
        return Self::word_break_recursive(&s, 0, s.len() as i32, &word_dict);
    }
}

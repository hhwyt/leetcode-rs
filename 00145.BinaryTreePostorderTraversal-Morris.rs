...
impl Solution {
    pub fn postorder_traversal(mut some_root: Option<Rc<RefCell<TreeNode>>>) -> Vec<i32> {
        let mut res = Vec::new();
        while let Some(rc_root) = some_root {
            let root = rc_root.borrow();
            match root.right.clone() {
                Some(mut cur) => {
                    while cur.borrow().left.is_some() && !Rc::ptr_eq(&cur.borrow().left.clone().unwrap(), &rc_root) {
                        cur = cur.clone().borrow().left.clone().unwrap();
                    }

                    if cur.borrow().left.is_none() {
                        res.push(root.val);
                        cur.borrow_mut().left = Some(rc_root.clone());
                        some_root = root.right.clone();
                    } else {
                        cur.borrow_mut().left = None;
